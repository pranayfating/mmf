...
def get_from_cache(
    ...
    # etag is None = we don't have a connection, or
    # url doesn't exist, or is otherwise inaccessible.
    ...
    if etag is None:
        if os.path.exists(cache_path):
            ...
            if len(matching_files) > 0:
                ...
                if local_files_only:
                    raise ValueError(
                        "Cannot find the requested files in the cached "
                        "path and outgoing traffic has been"
                        " disabled. To enable model look-ups and "
                        "downloads online, set 'local_files_only'"
                        " to False."
                    )
                return None
    ...
    with FileLock(lock_path):
        ...
        with temp_file_manager() as temp_file:
            print(
                "%s not found in cache or force_download set "
                + "to True, downloading to %s",
                url,
                temp_file.name,
            )
